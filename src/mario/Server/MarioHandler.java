...
public class MarioHandler extends Thread {
	...
	private String realname; // ▶2020.11.08 수정한 부분 (추가) 8
	private int seq; // ▶2020.11.08 수정한 부분 (추가) 8

	static List<MarioDTO> list_PlayerInfo;

	/*
	 * # 필드 서버클래스로부터 받은 소켓, 핸들러 리스트 dto객체정보 송수신 클래스 각각 핸들러에 닉네임 정보 저장하기 static 전체
	 * 플레이어 정보 리스트
	 */
...
//						System.out.println("MOVE:list_PlayerInfo : " + data.getNickname());
					...
					sendDTO.setNickname_array(array_nickname);
					sendDTO.setCoordinate(array_Coordinate);
...
//					}
					...
					System.out.println("[ DB로부터 받은 계정 정보 ]");
					for (MarioDTO data : dao.getMarioList()) {
						System.out.println(data.getClientAccount());
					...
					/* ******************************************************************* */
					/* ******************************************************************* */
				} else if (dto.getProtocol() == Protocols.UPDATE) { // ▶2020.11.08 수정한 부분 (추가) 5

					/* 받은 DTO를 DB에 입력 */
					MarioDAO dao = MarioDAO.getInstance();
					dao.updateArticle(dto);
					System.out.println("Protocols.UPDATE : DB로 전송 성공!");
					...
					/* ******************************************************************* */
				} else if (dto.getProtocol() == Protocols.DELETE) { // ▶2020.11.08 수정한 부분 (추가) 7

					int seq = this.seq;
					/* 받은 DTO를 DB에 입력 */
					MarioDAO dao = MarioDAO.getInstance();
					dao.deleteArticle(seq);
					System.out.println("Protocols.DELETE : DB로 전송 성공!");
					...
					/* ******************************************************************* */
				} else if (dto.getProtocol() == Protocols.TABLELOGIN) {

					/* DBTable에 회원정보 전체를 출력하는 메소드를 실행 */
					MarioDBTable.selectArticle();
					System.out.println("Protocols.TABLELOGIN : 실행성공!");
				...
				/*
				 * # 사용 빈도수가 높은 순서대로 MOVE > SEND > JOIN > EXIT
				 * 
				 *  # 프로토콜 MOVE : 클라이언트 좌표 송수신 
				 *  SEND : 메세지 송수신 
				 *  JOIN : 입장 
				 *  EXIT : 퇴장
				 *   CONNECT : DB에 저장된 회원정보 수신 
				 *   SINGUP : 회원가입 
				 *   UPDATE : 회원정보 수정 
				 *   DELETE : 회원정보 삭제(탈퇴)
				 * 
